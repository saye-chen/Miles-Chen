...
It helps operators, sellers, investors, and content teams answer seven practical questions:
...
The system contains seven independent but interoperable Codex skills:
...
├── D07 — Logistics, Inventory & Fulfillment Decision
│   └── routing, replenishment, allocation, fulfillment, returns, incidents, and overseas inventory exit
└── D08 — Platform, Store, Listing & Conversion
    └── listing, title, image set, detail, landing-page, funnel, experiment, incident, and migration decisions
```

This is not a generic research prompt library. CIDM owns capital decisions; CIM owns external competitive change; VLB owns content mechanisms and transferability; CIG owns first-party customer evidence and incremental growth; D09 owns advertising architecture, measurement, internal budget allocation, scaling, and stopping; D07 owns logistics networks, inventory, replenishment, fulfillment, reverse flow, and overseas inventory exit; D08 owns platform, store, Listing, title, image-set, detail-page, landing-page, and conversion decisions. Each skill can operate independently, while cross-skill evidence remains explicit, attributable, and reversible.

**D09 Runtime:** `D09-2026.07`

**D08 Runtime:** `D08-2026.01`

## Current Release Status

- Seven skills expose independent execution paths and professional report contracts.
- The executable full-repository expert audit currently scores **100/100**, including independent reports, cross-skill routing, extreme composite scenarios, multi-turn continuity, pressure gates, and release governance.
- D08 includes 12 platform expert cards, four-surface concrete optimization contracts, eight output Goldens, 100 semantically distinct scenarios, 13 deterministic tool families, and six-skill migration parity.
- D08 remains a **controlled pilot** until three authorized real historical replays pass its production gate; the repository does not describe that pending evidence as production validation.
...
按“要做什么决策”选择主 Skill，而不是按手头文件格式选择。七个 Skill 都能独立运行；跨 Skill 承接只在用户明确要求或提供上游报告时发生，不静默覆盖历史结论。
...
| **D09 — 广告分析、测量与优化** | 广告是否可投、问题在哪、真实增量多少、该放量还是停止？ | 广告架构、测量诊断、成熟利润、预算出价、实验和 Scale/Stop | 需要资本、库存、页面、内容、营销或客户证据共同裁决时 |
| **D07 — 物流、库存与履约决策** | 走什么路线和仓、何时补多少、库存怎么分、失败后如何收尾？ | 路由、补货、多仓分配、履约、退货、异常和海外库存退出 | 需要采购、合规、资金、渠道、广告或客户动作共同确认时 |
| **D08 — 平台、店铺、Listing与转化** | 页面能否承接商品和流量，标题、主图、详情、落地页具体怎么改？ | 页面诊断、具体改稿/制作Brief、漏斗根因、实验、恢复和迁移 | 需要产品、合规、利润、库存、广告、内容或客户证据共同确认时 |
...
flowchart TB
    ...
    R -->|"广告测量、预算与放量"| D09["D09<br/>广告分析与优化"]
    R -->|"路线、补货、库存与履约"| D07["D07<br/>物流库存与履约"]
    R -->|"平台、店铺、Listing与转化"| D08["D08<br/>页面承接与转化"]
    ...
    VLB -->|"内容变体与生产方案"| CIG
    CIDM -->|"资本边界、上市计划与退出姿态"| D07
    D07 -->|"库存可用性、履约容量、成本与退出风险"| CIDM
    D09 -->|"需求情景与活动节奏"| D07
    D07 -->|"可承诺流量、容量阻断与恢复窗口"| D09
    D09 -->|"流量结构、广告承诺与落地页问题"| D08
    D08 -->|"页面就绪度、转化证据与实验约束"| D09
    D07 -->|"ATP、履约时效、库存与退货约束"| D08
    D08 -->|"页面承诺、缺货信号与履约异常"| D07
    VLB -->|"内容资产、表达机制与制作约束"| D08
    D08 -->|"主图、详情与落地页制作 Brief"| VLB
    CIG -->|"VOC、人群任务、阻力与期望"| D08
    D08 -->|"页面信息缺口与预期偏差"| CIG
```
...
5. **D09 负责广告内部决策。** 页面、库存、利润与客户证据是外部门槛；D09 不越权替代资本、页面或履约主权。
6. **D07 负责物流库存与履约。** 可售库存、容量、路线、退货和退出门不能由广告需求或页面承诺覆盖。
7. **D08 负责页面承接与转化。** 诊断必须落到标题、主图、详情或落地页的具体改稿、制作 Brief、实验和停止/回滚条件；页面分数不证明商品、广告或库存决策正确。
8. **所有承接都必须显式、可逆。** 保留 Skill/模型版本、证据或事件 ID、数据截止日、原结论、建议调整、置信度和门槛影响，不静默改写历史报告。

### 常用完整链路

| 场景 | 推荐链路 | 最终由谁决断 |
|---|---|---|
| 新品类或新商品 | `CIDM → CIM → 按需 VLB/CIG/D07/D08 → CIDM` | CIDM 重判进入和验证；D07/D08 确认可履约、可承接 |
| 竞品、素材或页面异动 | `CIM → VLB/D08 → CIM → 必要时 CIDM` | CIM 管事件；VLB/D08 管内容与页面；CIDM 管资本影响 |
| 客户痛点或流失 | `CIG → 按需 CIDM/CIM/VLB/D08 → CIG` | CIG 验证客户影响与增量；D08 修复页面预期差 |
| 内容商业化 | `CIDM/CIG 提供上下文 → VLB → D08 承接 → CIG 实验 → CIDM` | VLB 管内容，D08 管页面，CIG 验证增量，CIDM 决定投资 |
| 有流量无转化 | `D09 → D08 → 按需 CIG/VLB/D07 → D09` | D08 定位并优化页面；D09 重判流量与预算 |
| 大促或直播放量 | `D09 → D07 容量与库存门 + D08 页面就绪门 → CIDM/D09` | D07 管履约门，D08 管页面门，CIDM 管资本，D09 管广告 |
| 上市后监控 | `CIM + CIG + D07 + D08 → CIDM；广告/内容事件再接 D09/VLB` | CIDM 决定放量、维持、复核、缩减或停止 |
...
| `video-link-breakdown/` | D11 content creative and propagation skill, modular analysis references, video preparation, and tests |
...
| `advertising-analysis-measurement-optimization/` | D09 skill, cross-platform routing, ad economics, nine-layer diagnosis, measurement, platform cards, and tests |
| `logistics-inventory-fulfillment-decision/` | D07 skill, routing, forecast routing, replenishment dates, min-cost multi-warehouse flow, warehouse capacity, fulfillment, reverse logistics, exit NPV, action lineage, historical replay gate, and executable tests (`D07-2026.03`) |
| `platform-store-listing-conversion/` | D08 skill for cross-platform store, Listing, title, image-set, detail-page, landing-page, funnel, experiment, incident, migration, 8 output contracts, concrete optimization, and deterministic validation (`D08-2026.01`) |
| `scripts/validate_repo.py` | repository-wide runtime, link, metadata, and scoring validation |
| `scripts/test_cross_skill_integration.py` | seven-skill cross-domain ownership, routing, lineage, and integration tests |
...
| `scripts/test_advertising_stress.py` | D09 platform depth, measurement, economics, marginal-budget, ownership, exclusion, and dynamic-fact stress tests |
| `scripts/test_logistics_stress.py` | D07 full-chain, ownership, dynamic-fact, failure-exit, deterministic-model, and 60-scenario stress tests; the Skill also runs executable-fixture, state/action-lineage, cross-skill, multi-turn-report, and historical-replay gates |
| `scripts/test_listing_conversion_stress.py` | D08 sovereignty, six gates, four-surface concrete optimization, 14 scenarios, 12 platform cards, 13 deterministic tools, lineage, 100-point acceptance, and real-replay production gate |
| `scripts/test_full_repository_audit.py` | executable 100-point audit for seven independent skills, reports, cross-skill scenarios, extreme composites, multi-turn continuity, pressure gates, and release governance |
...
| `scripts/validate_decision_contract.py` | shared seven-domain decision-contract entry point; every Skill exposes a local routed entry point |
...
Validation covers both Codex skill structure and repository-specific invariants. The current release gates cover all seven independent report contracts, deterministic calculations, cross-skill ownership and lineage, extreme composite scenarios, repeated follow-up continuity, 100-point expert-report acceptance, D08 four-surface concrete optimization, and controlled-pilot evidence requirements.
...
系统包含七项相互独立、又可以按需协同的 Codex 专业能力：
...
└── D08 — 平台、店铺、Listing与转化
    └── 标题、主图、详情、落地页、漏斗、实验、异常与迁移决策
```

它不是一组泛化研究提示词。CIDM 负责资本决策，CIM 负责外部竞争变化，VLB 负责内容机制与迁移性，CIG 负责一方客户证据与增量增长，D09 负责广告架构、测量、广告内部预算与放量停投，D07 负责物流网络、库存、补货、履约、逆向和海外库存退出，D08 负责平台、店铺、Listing、标题、主图、详情、落地页与转化承接。七项能力可以独立运行；跨 Skill 承接必须显式、可追溯、可逆。
...
- 七个 Skill 均具备独立执行入口与专业报告合同。
- 可执行全仓专家审计当前为 **100/100**，覆盖独立报告、跨 Skill 路由、极端复合场景、多轮追问连续性、压力门禁与发布治理。
- D08 已具备 12 个平台专家卡、四类页面具体优化合同、8 份输出 Golden、100 个语义不重复场景、13 类确定性工具及六 Skill 迁移对等校验。
- D08 在完成 3 份经授权的真实历史回放前仍为**受控试点**；仓库不会把尚未取得的真实证据表述为生产验证完成。
...
| **D09 — 广告分析、测量与优化** | 广告是否可投、问题在哪、该放量还是停止？ | 广告经济、九层诊断、测量、预算出价和实验 |
| **D07 — 物流、库存与履约决策** | 走什么路线和仓、何时补多少、库存怎么分？ | 路由、补货、多仓分配、履约、退货和退出 |
| **D08 — 平台、店铺、Listing与转化** | 页面能否承接流量，标题、主图、详情和落地页怎么改？ | 具体改稿/Brief、漏斗根因、实验、异常恢复和迁移 |

七个 Skill 各自保留专业主权：CIDM 管资本，CIM 管外部竞争，VLB 管内容机制，CIG 管一方客户与增量，D09 管广告内部决策，D07 管物流库存履约，D08 管页面承接与转化。任何局部信号或单项评分都不能自动升级为其他专业域的最终结论。
...
D09 判断广告是否具备投放条件，定位不消耗、高成本、低转化、有单无利润等问题，校验归因与真实增量，并决定广告内部预算如何放量、维持、降量或停止。它覆盖主流跨境站内、站外和 DTC 广告生态，也能对未知平台走通用路由，但不替代资本、库存、页面、内容、客户或合规主权。
...
D07 治理采购放行到可售库存的完整链路，包括路线与仓网、需求路由、ATP/CTP、补货与安全库存、多平台分配、仓内容量、尾程履约、退货召回、异常恢复、清仓与海外库存退出。报告必须把建议绑定到成本、约束、证据、动作、成功条件、停止/回滚条件和责任边界。
...
D08 针对主流跨境平台与 DTC 渠道的店铺、Listing、标题、主图/图集、详情页和落地页进行平台化深度诊断，未知平台走通用路由。交付不能停在评分或原则，必须给出具体改稿或制作 Brief、证据与约束、预期机制、实验设计、成功/停止/回滚条件；根因属于产品、合规、定价、库存、广告、内容或客户体验时必须显式承接。自动化专家门禁为 100/100；在 3 份经授权真实历史回放完成前仍保持受控试点。
...
| `video-link-breakdown/` | D11 内容创意与传播技能、模块化分析资料、视频准备工具与测试 |
...
| `advertising-analysis-measurement-optimization/` | D09 技能、跨平台路由、广告经济、九层诊断、测量、平台卡与测试 |
| `logistics-inventory-fulfillment-decision/` | D07 技能、物流路由、预测路由、补货日期、最小成本多仓流量、仓内能力、履约、逆向、退出NPV、动作血缘、历史回放门与可执行测试（`D07-2026.03`） |
| `platform-store-listing-conversion/` | D08 技能、全平台店铺与Listing、标题/主图/详情/落地页具体优化、12个平台专家卡、8类输出合同、100个语义场景、13类确定性工具与受控试点门（`D08-2026.01`） |
| `scripts/validate_repo.py` | 仓库级运行版本、链接、元数据与评分规则校验 |
| `scripts/test_cross_skill_integration.py` | 七 Skill 跨域主权、路由、血缘与集成测试 |
| `scripts/test_domain_stress.py` | 既有四项升级能力的标准、缺失、冲突、极端、主权、代理证据与确定性计算压力测试 |
| `scripts/test_advertising_stress.py` | D09 平台深度、测量、经济、边际预算、主权、排除项与动态事实压力测试 |
| `scripts/test_logistics_stress.py` | D07 全链路、主权、动态事实、失败退出、确定性模型与60场景压力测试；Skill同时执行fixture、状态/动作血缘、跨Skill、连续追问报告和历史回放闸门 |
| `scripts/test_listing_conversion_stress.py` | D08 主权、六道Gate、四类具体优化、14组场景、12个平台卡、13类确定性工具、血缘、100分验收和真实回放生产门压力测试 |
| `scripts/test_full_repository_audit.py` | 七Skill独立执行、专业报告、跨域、极端复合、多轮追问、压力与发布治理的可执行100分全仓审计 |
...
| `scripts/validate_decision_contract.py` | 七域共享决策合同入口；每个 Skill 均提供本地路由入口 |
...
质量验证同时覆盖 Codex Skill 基础结构与仓库专属不变量。当前发布门禁覆盖七 Skill 独立报告合同、确定性计算、跨 Skill 主权与血缘、极端复合场景、多轮追问连续性、专家报告 100 分验收、D08 四类页面具体优化和受控试点证据条件。
