...
It helps operators, sellers, investors, and content teams answer five practical questions:
...
The system contains six independent but interoperable Codex skills:
...
按“要做什么决策”选择主 Skill，而不是按手头文件格式选择。六个 Skill 都能独立运行；跨 Skill 承接只在用户明确要求或提供上游报告时发生，不静默覆盖历史结论。
...
| 场景 | 推荐链路 | 最终由谁决断 |
|---|---|---|
| 新品类或新商品 | `CIDM → CIM → 按需 VLB/CIG/D07 → CIDM` | CIDM 重判进入和验证；D07 确认可履约性 |
...
| 大促或直播放量 | `D09 → D07 容量与库存门 → CIDM/D09` | D07 拥有库存履约门；CIDM 管资本，D09 管广告 |
| 上市后监控 | `CIM + CIG + D07 → CIDM；内容事件再接 VLB` | CIDM 决定放量、维持、复核、缩减或停止 |
...
| `scripts/validate_decision_contract.py` | shared six-domain decision-contract entry point; every Skill exposes a local routed entry point |
...
系统包含五项相互独立、又可以按需协同的 Codex 专业能力：
...
它不是一组泛化研究提示词。CIDM 负责资本决策，CIM 负责外部竞争变化，VLB 负责内容机制与迁移性，CIG 负责一方客户证据与增量增长，D09 负责广告架构、测量、广告内部预算与放量停投，D07 负责物流网络、库存、补货、履约、逆向和海外库存退出。六项能力可以独立运行；跨 Skill 承接必须显式、可追溯、可逆。
...
| `scripts/validate_decision_contract.py` | 六域共享决策合同入口；每个 Skill 均提供本地路由入口 |
